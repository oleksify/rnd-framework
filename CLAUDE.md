...
Notable skills: `rnd-roadmapping` (roadmap.md format + milestone lifecycle), `rnd-learning` (auto-capture pipeline gotchas to `$CLAUDE_CONFIG_DIR/learnings/`), `rnd-formatting` (detect+run project formatter pre-commit), `rnd-language-design` (DSL/parser/compiler design packages with empirical specs — accepted/rejected examples, golden ASTs, diagnostics fixtures; conditionally loaded by rnd-audit/rnd-review/rnd-debug/rnd-brainstorm/rnd-decomposition when the work touches a DSL, grammar, parser, interpreter, compiler, renderer, executor, or validator), `rnd-explain` (drives `/rnd-framework:rnd-explain`; fills a fixed canonical `template.html` — inline CSS design system + data-driven quiz engine — to emit a self-contained, interactive HTML explanation of a diff/branch/PR (Background/Intuition/Code/Quiz), so design is deterministic and the quiz wiring is never re-authored per run).
