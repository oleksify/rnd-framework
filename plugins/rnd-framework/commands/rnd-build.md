...
- If a clear next wave is found, use `AskUserQuestion`/`AskUser` to confirm: "Build wave-N next? (N tasks: T1, T2, ...)" with options "Yes, build wave-N (Recommended)" and "Choose a different task or wave".
- If no pending wave is found, report the current state and use `AskUserQuestion`/`AskUser`.
...
| `NEEDS_CONTEXT` | Pause. Use `AskUserQuestion`/`AskUser` to collect missing context. Resume with user's answer. |
| `BLOCKED` | Pause. `AskUserQuestion`/`AskUser`: "Provide missing dependency manually", "Re-plan this task", "Skip this task". |
...
Summarize build results. Then use `AskUserQuestion`/`AskUser` with options:
