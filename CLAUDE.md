...
│   ├── self-assessment-producer.sh / shape-producer.sh / calibration-producer.sh   # PostToolUse Write|Edit path-driven Phase 0 stat producers (identity read off the artifact file_path via lib.sh helpers, NEVER .current-session). self-assessment.md write→{event:builder_self_assessment,task_id,self_verdict} (the filename stem is resolved to the canonical features.json id via the unique M<N>.T<NN> prefix — exact-match→prefix-match→raw-stem fallback — so a truncated/drifted slug or a bare slot still emits the joinable id); validation-contract.md OR features.json write (both-present gated)→one {event:assertion_shape,task_id,assertion_id,shape} per assertion mapped to its owning task via features.json — both to the session audit.jsonl with snake_case task_id; wave-<N>-verdict-map.json write→per-task {task_id,verdict} collapsed via the Gate 3 rule to the slug-root calibration.jsonl. The task identifier is snake_case task_id across audit, verdict-map, AND calibration (unified); the stats views read COALESCE(task_id, taskId) so historical camelCase calibration records still join. Non-blocking. (Replaced the old builder-self-assessment-emit.sh SubagentStop hook, which mis-resolved the session via .current-session and emitted 0 events.)
...
│   ├── stats/*.sql                      # Stateless DuckDB view module over session JSONL in place (tolerant `read_csv` raw-line + `json_valid`, no persistence): shape distribution, per-shape verifier-FAIL rate, iteration depth, builder-self-fail-vs-verdict gap, FAIL-rate drift, sycophancy flip rate (over `*/sycophancy-probe.jsonl`, per-`artifact_basis` hard/soft split, with the clean hard-flip rate scoped to `statically_verifiable='true'` rows and execution/multi-file rows reclassified — never dropped — into a not-statically-re-verifiable bucket), drift watch (per-segment rolling 10-session regr_slope of iteration metric and replan frequency), post-review findings (`post_review_findings.sql`: per-(segment,shape) post-pipeline review count, finding count, and verifier-said-PASS-vs-review-found gap over `*/post-review.jsonl`, per-(session,shape) grain via `bool_or` so a multi-finding session is one dirty run), + backfill.sql; segment via inline `dogfood_slugs` CTE; run out-of-band by /rnd-framework:rnd-stats (Section 6 = sycophancy, Section 7 = drift watch, Section 8 = post-review findings)
│   ├── remeasurement.sh                 # M7 re-measurement harness: subcommands `corpus_count [boundary_epoch]` (counts dogfood session dirs newer than the M5 ship boundary) and `gate_met [boundary_epoch]` (exit 0 at N≥10, exit 1 below); `memo <out_path> [boundary_epoch]` writes a pending stub when the gate is unmet or queries stats/per_shape_fail_rate + self_fail_vs_verdict_gap + iteration_depth and emits a markdown memo with M3-baseline-recall + current-snapshot + delta + M4+M5 confound disclosure + follow-up signals sections. The corpus boundary is the baked-in `M5_EPOCH=1779963467` constant (v5.6.0 commit 62311e9), NOT a git-resolved SHA — the optional arg accepts only a positive epoch override (used by tests) and fails loud on anything else, so an unresolvable boundary can never silently degrade to counting the whole corpus. Read-only over the existing M1 substrate; invoked by /rnd-framework:rnd-remeasure.
...
│   ├── post-review-writer.sh           # Post-SHIP review (Phase 8) record writer: appends one per-finding JSON line {shape,severity,verifier_said_PASS,review_found,session_id,timestamp} to the slug-root `post-review.jsonl`; finding shape attributed mechanically (touched-file → owning task via the `M<NN>-T<NN>-<uuid>` manifest filename's uuid → features.json → first assertion_shape in audit.jsonl), unowned findings recorded `shape:"unattributable"`; `--clean-shape <shape>` mode emits one clean row (review_found:false, severity:"none") per in-scope shape for a clean review (so post-review.jsonl is self-contained for the per-shape validity ledger)
│   └── calibration.sh                  # Auto-escalation helpers; subcommands: window, false_pass_rate, should_promote, promote_tier, task_type_window; PLUS the per-shape validity ledger — `consecutive_clean <shape>` (collapses post-review.jsonl per-finding rows to one clean/dirty verdict per (session,shape), counts the trailing consecutive-clean streak) and `validity <shape>` (exit 0 + "expert" iff streak ≥ N_EXPERT_CONSECUTIVE_CLEAN=5, else "novice <n>"; stateless recompute each call → one-strike demotion falls out automatically)
└── README.md
```
...
└── branches/<branch>/                     # Branch-scoped partition (slashes → nested dirs)
    ...
    └── sessions/<YYYYMMDD-HHMMSS-XXXX>/   # $RND_DIR (one per pipeline run)
        ...
        ├── builds/M<NN>-T<NN>-<uuid>-manifest.md  # Terse build records (structured bullets, no narrative); named by the collision-proof task reference (zero-padded milestone + task + the features.json `uuid`) so a `T<NN>` repeated across milestones never overwrites; the uuid is the exact attribution join key read by post-review-writer.sh (no substring matching)
