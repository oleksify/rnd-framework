...
│   ├── evidence-locking-gate.sh        # M6: PreToolUse Write|Edit, verifier-scoped. Blocks the verifier's wave-N-verdict-map.json write when any assertion entry has an empty, missing, or trivial (every-not-any) evidence array. Single jq pass; reads $new_content from PreToolUse tool_input (file not yet written); one batched gate_fired event with first-offender ID. Form-only — does not substance-check citations.
...
│   ├── verdict-map-schema.json         # M6: draft-07 SSOT for the verifier verdict-map evidence-array shape; `x-trivial-tokens` (19-item denylist), `x-min-evidence-length` (40), and `x-evidence-citation-markers` (5 markers) sourced at runtime by evidence-locking-gate.sh
...
- **Evidence locking gate** (`evidence-locking-gate.sh`): M6 PreToolUse Write|Edit; scoped to `rnd-verifier` writing `$RND_DIR/verifications/wave-*-verdict-map.json`. Sources `x-trivial-tokens` / `x-min-evidence-length` / `x-evidence-citation-markers` from `lib/verdict-map-schema.json` SSOT (hardcoded fallback if schema unreadable). One streaming jq pass walks every assertion entry; an evidence item is **non-trivial** when length ≥ 40 OR it contains any citation marker (`:` `/` `` ` `` `"` `<`) — trivial-tokens denylist applies only when neither structural test passes. Every-not-any semantics: any single trivial item fails the entry. On offender, emits one `gate_fired` event (`tool: evidence_locking_gate`, `task_id: <first offender>`) then blocks with stderr naming the violation type. Form-only: does NOT grep evidence strings against session artifacts for substance — that's deferred work. Coexists additively with `write-gate.sh`'s `.rnd/` auto-allow.
