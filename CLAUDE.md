...
├── agents/                             # 14 agents (10 pipeline-phase + 4 helpers: premortem fan-out + replan-differ + assertion paraphraser + read-only explorer)
...
│   ├── self-assessment-producer.sh / shape-producer.sh / calibration-producer.sh   # PostToolUse Write|Edit path-driven Phase 0 stat producers (identity read off the artifact file_path via lib.sh helpers, NEVER .current-session). self-assessment.md→{event:builder_self_assessment,task_id,self_verdict} (stem resolved to canonical features.json id via unique M<N>.T<NN> prefix: exact→prefix→raw-stem fallback); validation-contract.md OR features.json (both-present gated)→{event:assertion_shape,task_id,assertion_id,shape} per assertion; wave-<N>-verdict-map.json→per-task {task_id,verdict} collapsed via Gate 3 to slug-root calibration.jsonl. task_id is unified snake_case across audit/verdict-map/calibration; stats views COALESCE(task_id, taskId) for historical camelCase. Non-blocking.
...
│   ├── stats/*.sql                      # Stateless DuckDB view module over session JSONL in place (tolerant `read_csv` + `json_valid`, no persistence): shape distribution, per-shape verifier-FAIL rate, iteration depth, builder-self-fail-vs-verdict gap, FAIL-rate drift, sycophancy flip rate (per-`artifact_basis` hard/soft split; clean hard-flip scoped to `statically_verifiable='true'`, execution/multi-file rows reclassified not dropped), drift watch (per-segment rolling 10-session regr_slope), post-review findings (`post_review_findings.sql`: per-(segment,shape) review/finding count + verifier-PASS-vs-review gap, per-(session,shape) grain via `bool_or`), + backfill.sql; segment via inline `dogfood_slugs` CTE; run by /rnd-framework:rnd-stats (§6 sycophancy, §7 drift, §8 post-review)
│   ├── remeasurement.sh                 # M7 re-measurement harness: `corpus_count [epoch]` (dogfood session dirs newer than M5 boundary), `gate_met [epoch]` (exit 0 at N≥10), `memo <out> [epoch]` (pending stub if gate unmet, else markdown memo: M3-baseline-recall + current-snapshot + delta + M4+M5 confound disclosure + follow-up signals). Boundary is baked-in `M5_EPOCH=1779963467` (v5.6.0 commit 62311e9), NOT git-resolved; optional arg accepts only a positive epoch override and fails loud otherwise. Read-only; invoked by /rnd-framework:rnd-remeasure.
...
│   ├── audit-scan.sh                   # Subcommands: `verdict_history <task>` (verification reports sorted by mtime then path; prints FLIP_DETECTED on PASS→non-pass→PASS or non-pass→PASS→non-pass, where non-pass = FAIL | NEEDS_ITERATION | PASS_QUALITY_NEEDS_ITERATION; consumed by rnd-debug's Step 4 flip stop condition)
...
│   ├── sycophancy-probe.sh             # One-shot sycophancy-delta probe: `prepare` reconstructs each historical PASS assertion's artifact at its session's commit via guarded `git show` (artifact_basis pinned_commit|head_fallback), writing barrier-clean review-inputs (assertion + artifact only); `ingest` appends `{assertion_ref,session_id,commit_sha,artifact_basis,new_verdict,hard_flip,soft_flip,rationale,statically_verifiable}` per re-review to slug-root `sycophancy-probe.jsonl` (rationale/statically_verifiable backfill-safe, drive stats segmentation); `summary` prints corpus/reviewed/dropped + pinned/fallback split. Orchestrator-driven, never hot path
│   ├── post-review-writer.sh           # Post-SHIP review (Phase 8) writer: appends per-finding {shape,severity,verifier_said_PASS,review_found,session_id,timestamp} to slug-root `post-review.jsonl`; shape attributed mechanically (touched-file → owning task via `M<NN>-T<NN>-<uuid>` manifest uuid → features.json → first assertion_shape), unowned → `shape:"unattributable"`; `--clean-shape <shape>` emits one clean row per in-scope shape (so the ledger is self-contained)
│   └── calibration.sh                  # Auto-escalation helpers: window, false_pass_rate, should_promote, promote_tier, task_type_window; PLUS per-shape validity ledger — `consecutive_clean <shape>` (collapses post-review.jsonl to one verdict per (session,shape), counts trailing clean streak) and `validity <shape>` (exit 0 + "expert" iff streak ≥ N_EXPERT_CONSECUTIVE_CLEAN=5, else "novice <n>"; stateless recompute → automatic one-strike demotion)
└── README.md
```
...
Fourteen specialized agents (10 pipeline-phase + 4 helpers) handle pipeline work in isolated context windows. The orchestrator dispatches work to agents, enforcing structural information barriers — the Verifier literally cannot see the Builder's reasoning because they run in separate context windows.
...
**Validity-Gated Expertise ("earn the fast path").** A second dispatch input, layered on criticality. After the final SHIP, **Phase 8** auto-runs the post-SHIP review (opt-out via `--skip-post-review`, which emits a measurable skip audit event) and `post-review-writer.sh` records its findings — plus per-in-scope-shape clean rows — to the slug-root `post-review.jsonl`. The per-shape validity ledger (`calibration.sh validity <shape>`) recomputes, on each call from that ledger alone, the trailing consecutive-clean streak; a shape is **expert** at ≥5 (mirroring `N_THIN_CORPUS`). The **Shape-Validity Fast Path** gate (in `rnd-orchestration` + `commands/rnd-start.md`, structured like the `should_promote` gate) emits a *fast profile* — recognition-primed lighter builder, prose/reduced-experiment verifier, single build-verify pass — **iff** the task's shape is expert AND criticality ∈ {LOW, NORMAL}; models stay at the criticality tier. The no-slop floor is inviolable: **verification always runs** (lighter, never absent), the fast builder **still writes its manifest** (load-bearing for attribution), **HIGH criticality never fast-paths** (hard floor), and **one-strike demotion** is automatic — because the streak is recomputed statelessly, any new post-review finding mechanically drops the shape below expert with no shadow record. Kahneman–Klein (2009): fast intuition is trustworthy only in a regular environment with feedback — so speed is *earned* per-shape by a feedback-confirmed track record, never assumed.
...
- **Information barrier** (`read-gate.sh`, `glob-grep-gate.sh`, `bash-gate.sh`): for `rnd-verifier`/`rnd-polisher`, blocks any tool call whose path/command contains `self-assessment.md` or `self-assessment-properties`, OR matches `.rnd/.*briefs/`, OR `.rnd/.*cleanup/`. The `self-assessment` patterns match the artifact-specific tokens, NOT the bare substring, so source files like `hooks/self-assessment-producer.sh` are not false-positives. The orchestrator (empty `agent_type`) is the legitimate consumer of briefs/ + self-assessment and is NOT barrier-restricted. The `.rnd/` anchor on `/briefs/` + `/cleanup/` distinguishes artifact-tree paths from same-named project-source dirs. `glob-grep-gate.sh` also checks `path`+`pattern` concatenated so a split glob can't smuggle through.
...
Notable skills: `rnd-roadmapping` (roadmap.md format + milestone lifecycle), `rnd-learning` (auto-capture pipeline gotchas to `$CLAUDE_CONFIG_DIR/learnings/`), `rnd-formatting` (detect+run project formatter pre-commit), `rnd-language-design` (DSL/parser/compiler design packages with empirical specs — accepted/rejected examples, golden ASTs, diagnostics fixtures; conditionally loaded by rnd-audit/rnd-review/rnd-debug/rnd-brainstorm/rnd-decomposition when the work touches a DSL, grammar, parser, interpreter, compiler, renderer, executor, or validator), `rnd-explain` (drives `/rnd-framework:rnd-explain`; emits a self-contained, interactive HTML explanation of a diff/branch/PR built on an inlined, runtime-rendered Markdoc document — the model authors only a single Markdoc island (prose + typed `{% tag %}` custom components: callout, chart, diagram, stepper, formula, kpi, mock) plus a 5-question quiz, and a fixed template parses/validates/renders it in-browser at load across the same four-section arc (Background/Intuition/Code/Quiz), so design and quiz wiring stay deterministic).
...
├── post-review.jsonl                      # Slug-root post-SHIP review ledger (sibling to calibration.jsonl): per-finding rows + per-in-scope-shape clean rows {shape,severity,verifier_said_PASS,review_found,session_id,timestamp}; written by post-review-writer.sh during Phase 8; the ground truth for the per-shape validity ledger (calibration.sh `validity`) and rnd-stats Section 8
└── branches/<branch>/                     # Branch-scoped partition (slashes → nested dirs)
    ...
    └── sessions/<YYYYMMDD-HHMMSS-XXXX>/   # $RND_DIR (one per pipeline run)
        ...
        ├── outside-view.md                # Orchestrator-owned; written by lib/outside-view.sh BEFORE the Planner spawn (Phase 1, between premortem and Planner); the `## Outside View (Reference Class)` block is injected via ${OUTSIDE_VIEW_BLOCK}; post-emit writes outside_view_injected {mode, n_total, shapes, framing_constraint_emitted}
        ├── scope.json                     # Frozen deliverable SSOT; written by rnd-scoper after user ratification; consumed by rnd-planner; immutable once ratified
        ├── scope.md                       # Human-readable scope boundary presented to the user for ratification; immutable once approved
        ...
        ├── builds/M<NN>-T<NN>-<uuid>-manifest.md  # Terse build records (structured bullets); collision-proof name (zero-padded milestone+task+features.json `uuid`) so a `T<NN>` repeated across milestones never overwrites; uuid is the exact attribution join key for post-review-writer.sh
        ├── builds/<M<N>.T<NN>.slug>-self-assessment.md  # Builder uncertainties (blocked from Verifier); stem is the canonical features.json id so the emitted task_id joins calibration
        ...
        ├── verifications/T*-pass-receipt.json      # Minimal {status, source, timestamp} receipt the Verifier writes per PASS task; rnd-status/rnd-start read it to mark tasks verified
