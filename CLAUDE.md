...
│   └── calibration.sh           # Calibration auto-escalation helpers: window/false_pass_rate/should_promote/promote_tier subcommands; orchestrator invokes should_promote before each adaptive-agent spawn to react to model-quality drift; RND_DISABLE_AUTO_ESCALATION=1 disables
...
| Phase | Agent | Purpose |
|---|---|---|
| Planning | `rnd-planner` (opus/high, adaptive) | Decomposes tasks into pre-registered sub-tasks with testable criteria; capped at max 4 tasks/wave with min 1-hour scope and forced coalescing |
