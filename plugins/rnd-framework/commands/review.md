...
**Spawn 2 verifier agents in parallel** using the Agent tool with `subagent_type: "rnd-framework:rnd-verifier"` and `mode: "bypassPermissions"`. Each receives the same review prompt independently.
...
- **Disagree** — spawn a tiebreaker: a third agent with `subagent_type: "rnd-framework:rnd-verifier"` and `mode: "bypassPermissions"`, passing the same review prompt plus both prior reports (Judge A and Judge B). Save the tiebreaker report to `$RND_DIR/review/tiebreaker.md`. The tiebreaker's verdict is the final verdict.
