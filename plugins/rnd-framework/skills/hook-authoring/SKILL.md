...
- **Using sed/awk inside hooks** — avoid them even though they're not blocked at the gate level. Use `jq` for JSON and `lib.sh` functions for path checks — more readable and purpose-built for hook contexts.
