...
| Verification | `rnd-verifier` (sonnet/high, adaptive) | Wave-batched: one spawn per wave reviews all task pre-regs and emits a per-task verdict map; writes `T<id>-verification.md` full prose report for every verdict (PASS, FAIL, NEEDS_ITERATION, PASS_QUALITY_NEEDS_ITERATION, AMEND_REQUIRED); AMEND_REQUIRED (emit only with cited concrete spec defect; routes to amendment arbiter; clean-slate re-verification afterward) pauses the task without blocking the wave; information barrier enforced; HIGH criticality routes through wave-batched multi-judge with verdict-based escalation gate (single first-pass verifier; only FAIL/NEEDS_ITERATION/PASS_QUALITY_NEEDS_ITERATION/AMEND_REQUIRED escalates to full dual-judge; set `RND_MULTI_JUDGE_ALWAYS=1` to bypass gate and restore exact pre-gate always-dual-judge behavior) |
| Amendment | `rnd-amendment-arbiter` (sonnet/high) | Evaluates AMEND_REQUIRED verdicts; proposes spec corrections (AMEND), recommends rebuild (REBUILD), or routes to Planner re-plan (ESCALATE_REPLAN); inputs strictly limited to original pre-reg + Verifier verdict |
...
**Dispatch Policy (criticality-driven, adaptive agents only):** the orchestrator overrides the spawned agent's model based on the per-task `Criticality` field in the pre-registration — `LOW` → `sonnet`, `MEDIUM` → `sonnet`, `HIGH` → `opus`. All criticality tiers for adaptive core agents use `sonnet` as the floor; the `rnd-integrator` is the only agent that uses a lighter model, and it is non-adaptive. If `Criticality` is absent (or no pre-reg exists), the orchestrator does NOT override and the agent's frontmatter `model:` is used as the fallback. Effort is NOT per-spawn overridable; it stays at the agent's frontmatter value. Non-adaptive agents always use their frontmatter `model:`. Full policy lives in `rnd-framework:rnd-orchestration` skill under "Dispatch Policy".
